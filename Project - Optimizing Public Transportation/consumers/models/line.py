...
class Line:
    ...
    def process_message(self, message):
        ...
        if topic_name == "transformed_stations_table": # Set the conditional correctly to the stations Faust Table. Might need to change this to "transformed_stations"
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif topic_name == 'arrivals': # Set the conditional to the arrival topic
